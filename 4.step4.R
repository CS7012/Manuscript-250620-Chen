...
setwd("F:\\包\\imctxtdf2")
id1=read.table("result.txt",header =T,sep = "\t")
iddf=read.table("immresult.txt",header =T,sep = "\t")

gmid=as.vector(id1$id)

bioid=as.vector(iddf$id)

result=data.frame()
foreach(i=gmid, .errorhandling = "pass") %do%{
foreach(j=bioid, .errorhandling = "pass") %do%{
...
outc_rt <- read_outcome_data(
  snps = expo_rt$SNP,
  filename = paste0("imcall/",j,".txt.gz"),
  sep = "\t",
  snp_col = "SNP",
  beta_col = "beta",
  se_col = "se",
...
if (mr_result$pval[3]<0.05){
  result=rbind(result,cbind(id=i,id2=j,pvalue=result_or$pval[3])) 
  filename=paste0("bc_imc/",i,"_",j)
  dir.create(filename) 
